...
symmetric kernels that are also separable \cite{gonzalez1992digital}.

to apply, for example, a 1D \gls{GF} horizontally and then vertically
(or vice versa) instead of doing a 2D convolution, which is
faster. For example, the speed-up provied by separability in the 2D
case is (in terms of \gls{NoO})
\begin{equation}
  \text{Speed-up} = \frac{\text{NoO~in~the~2D~Convolution}}{\text{NoO~in~1D-convolving~the~rows}+\text{NoO~in~1D-convolving~the~columns}} = \frac{MNK^2}{2MNK} = \frac{K}{2},
\end{equation}
where $M$ and $N$ are the width and height of the filtered image, and
$K$ is the length of the Gaussian kernel.

\section{Denoising using \gls{GF}}
%{{{

Gaussian kerners are low-pass filters, and the \gls{SNR} is usually
smaller in the high frequencies. Therefore, when we filter a noisy
signal using \gls{GF} what we are basically eliminating is noise.
