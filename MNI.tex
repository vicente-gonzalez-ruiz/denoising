...
$n$-th noise sample (an image, for example) of
$\{\mathbf{n}\}_{n=0}^{N-1}$, and $\hat{\mathbf{n}}^{(n)}$ is the
$n$-noisy instance of $\{\hat{\mathbf{s}}^{(n)}\}_{n=0}^{N-1}$, the
Central Limit Theorem (CLT) states that the distribution of the sample
mean (or sum) of a sufficiently large number $N$ of independent and
identically distributed (i.i.d.) random variables (noise-signals in
our case) will approximate a normal distribution, regardless of the
original distribution of the individual variables
\cite{kendall1968advanced} (always that the variance is finite). In
maths,
\begin{equation}
