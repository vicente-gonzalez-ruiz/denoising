...
where now
\begin{equation}
  \text{GAT}(\hat{\mathbf{s}}_i)\sim\mathcal{N}(\mu',\approx 1).
\end{equation}
...
CLT implies that we only need to take into consideration the gaussian additive noise case when we perform NMI.
