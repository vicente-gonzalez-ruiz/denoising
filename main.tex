...
\begin{tabular}{ll}
  ...
  $\tilde{\mathbf{x}}^{(I)}$ & Approximation to $\mathbf{x}$ using the mean of $\{\hat{\mathbf{x}}^{(i)}\}_{i=0}^{I-1}$ \\ 
  ...
  $\mathcal{F}$ & The (forward) Fourier transform ($\mathcal{F}(\mathbf{x})=\mathbf{X}$) \\
