...
Mathematically, we will model signals as random variables. A random
variable is a mathematical formalization of a quantity or object which
depends on random events. When more than one value is generated in one
of these envents, we can establish a similar connection between
multicomponent signals and vector random variables.

In the case of multicomponent signals, ${\mathbf s}$ is usually
assumed to be drawn from a multivariate normal distribution:
\begin{equation}
  \mathbf{s}~\mathcal{N}(0,\Lambda),
\end{equation}
where $\Lambda$ is the covariance matrix of $\mathbf{s}$ which
describes the structure of the signal (see
Section~\ref{sec:covariance}).
...
removing noise, as it is our case, we will speak about denoising using
the WF.
...
Eq.~\ref{eq:WF_objective} (in other words, we can only estimate
...
The covariance matrix, usually denoted by $\Sigma$, is a square matrix
that summarizes the relationships between two (multivariate)
random variables. The elements along the main diagonal of the
covariance matrix represent the variance (see
Section~\ref{sec:covariance}) of each individual variable, i.e.
\begin{equation}
  \text{Var}(X_i)=\mathbb{E}[(X_i-\mathbb{E}[X_i])^2].
\end{equation}
and the
off-diagonal elements (he element at position $(i,j)$ (where
$i\neq j$) represents the covariance between the $i$-th and the $j$-th
variables:
\begin{equation}
...
defined by is a square matrix that  in which 
