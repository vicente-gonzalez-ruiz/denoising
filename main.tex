...
enought large sample size of noisy instances $\hat{\mathbf{X}}^{(i)}$,
it holds that
\begin{equation}
  \hat{\mathbf{X}}^{(i)} = \mathbf{X} + \mathbf{N}^{(i)},
\end{equation}
where $\mathbf{X}$ is the clean signal.

Notice that the even if the noise is signal dependent, for a given
sample $\hat{\mathbf{X}}^{(i)}_j$, the average (arithmetic mean) for
all an infinite number of $i$ instances will become $\mathbf{X}_j$. Because this is imposible physically, our \emph{clean} signal in practice will be always a $\tilde{\mathbf{X}}^{(I)}$ which will depends on $I$.
...
noise. Notice however, that in practice the GT is never available, and
we should always speak about \emph{estimations}.

In all of the following distortion metrics definitions we will
consider al ``clean'' (considered GT) signal $\mathbf{X}$ and an
approximation $\mathbf{Y}$, both with $J$ samples, likely generated by
some denoising algorithm. If $\mathbf{X}$ is not avaliable, these
...
but is very sensitive to shifts or misalignments.
...
underlying clean signal, $\hat{\mathbf{X}}^{(1)}$ and
$\hat{\mathbf{X}}^{(2)}$.
...
Unfortunately, the existence of two noisy instances of the clean
signal not always is possible in practice.\footnote{Particularly in
  contexts where samples are susceptible to the degradation caused by
  the microscope radiation.} In this case, a workaround is to simlate
such existence, by distributing the information of
$\hat{\mathbf{X}}^{(1)}$ between two noisy (sub-)instances
$\hat{\mathbf{X}}^{(1)}_1$ and $\hat{\mathbf{X}}^{(1)}_2$. For
example, in \cite{verbeke2024self} a even/odd signal splitting (see
...
resembles the true FC in some specific cases. In
\cite{koho2019fourier}, the curves must be calibrated depending on the
correlation. Therefore, in general, we should always consider the SFC
curve as an estimation of the true FC curve.
