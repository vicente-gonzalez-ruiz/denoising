...
between the denoised and true signal. It's optimal under the
assumption that the signal and noise are both random processes with
known power spectra, and that the noise is additive, and therefore,
noise and signal are uncorrelated. Therefore, if $N$ ... 
...
\begin{equation}
  \mathbf{X}_f=\sum_n\mathbf{x}_ne^{-2\pi jfn} = \sum_n\mathbf{x}_ne^{-jwn},
\end{equation}
