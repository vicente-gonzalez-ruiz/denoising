...
noise. Notice that if the GT signal is not available, these metrics are
not applicable, and that, in any case, we always should speak about \emph{estimations}, because the GT always (except if it is synthetic) contains some noise.
...
can help to determine resolution of the signal, for example, the
maximum spatial resolution that a microscope can achieve. In
microscopy, we usually speak about the \emph{resolving power}, which
is the smallest distance $d$ between two points on a specimen that can
still be distinguished as separate entities. The idea is, any
frequency above $1/d$ will always be dominated by noise, and
therefore, a low-pass filtering with a cut-off frequency of $1/d$
should only remove noise. Notice that $d/2$ should be the sampling
step size used to digitalize the signal.
...
The SSNR can be estimated using the correlation in the Fourier domain between two (or more, in this case, using the mean SSNR) instances $\hat{\mathbf{X}}^{(1)}$ and $\hat{\mathbf{X}}^{(2)}$. For the 3D case (and only for two instances), we define \cite{verbeke2024self,unser1987new}
\begin{equation}
