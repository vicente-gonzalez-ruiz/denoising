...
\begin{equation}
  \mathbf{H}_k = \frac{\mathbb{V}(\hat{\mathbf{X}}_k)}{\mathbb{V}(\hat{\mathbf{X}}_k) + \sigma^2},
  \label{eq:WF_coeffs}
\end{equation}
where $\mathbb{V}(\hat{\mathbf{X}}_k)$ is the variance of the $k$-th
Fourier coefficient of the $\text{DFT}(\hat{\mathbf{x}})$ over a
collection of $\hat{\mathbf{x}}$ instances, and $\sigma^2$ is (an
...
\item The noise power, expressed as the average variance of the noise
  which by default is
  $\sigma^2_i= \mathbb{E}\left(\mathbb{V}(\hat{\mathbf{x}}_{\{i\}})\right)$,
  where $\hat{\mathbf{x}}_{\{i\}}$ is the window used in the denoising
  process.
\item The window size $w$.
\end{enumerate}
In general, $\sigma^2_i$ is unknown, and therefore, the denoising
process should be controlled with $w$ (the larger $w$, the greater the
filtering effect).

If the GT signal were known, the noise power\footnote{Remember that the
  noise is additive and therefore, uncorrelated with the GT signal,
  the power of the noise is the same in all the windows.} can be found
as
\begin{equation}
  {\sigma^2}^* = \mathbb{E}\left(\mathbb{V}(\hat{\mathbf{x}}-\mathbf{x})\right),
\end{equation}
and we should set $w=1$ (we do not need to estimate the noise power).

In the rest of this document, we will refer to this filter as
``scipy.Wiener''.

Another alternative to perform Wiener denoising is to estimate the
transfer function of the filter through the SFC of the noisy signal
...
In this case, if the GT were known, Eq.~\ref{eq:WF_SFC} becomes
\begin{equation}
