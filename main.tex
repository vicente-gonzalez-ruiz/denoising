...
$\mathbf{s}^{()}$ between two noisy (sub-)instances
$\hat{\mathbf{s}}^{(1)}$ and $\hat{\mathbf{s}}^{(2)}$. For
...
\begin{figure}
  ...
  \caption{SFRC (Self Fourier Ring Correlation) of two clean images
    ...
    FSC(SPRS($\mathbf{s}$), SPRS($\mathbf{s}$))), and SPRS2 (computing
    the FSC($\mathbf{s}$, SPRS($\mathbf{s}$))). $f_s$ represents the
...
\begin{figure}
  ...
  \caption{SFRC (Self Fourier Ring Correlation) of two noisy images
    \cite{zhang2019poisson} using different data-splitting/shuffling
    algoritms: EOS (Even-Odd Splitting), CBS (ChessBoard Splitting),
    ICBS (Interpolated CBS), SCBS (Subsampled CBS), SPRS1 (Structure
    Preserving Random Shuffling computing the
    FSC(SPRS($\mathbf{s}^{()}$), SPRS($\mathbf{s}^{()}))$), and SPRS2
    (computing the FSC($\mathbf{s}^{()}$,
    SPRS($\mathbf{s}^{()}$))). For the purpose of comparison, it has
    also been show the FRC using another noisy instance of
    $\mathbf{s}$ (FSC($\mathbf{s}^{(1)}$, $\mathbf{s}^{(2)}$)). $f_s$
    is the sampling frequency.\label{fig:SFC_vs_splitting_noisy}}
\end{figure}
