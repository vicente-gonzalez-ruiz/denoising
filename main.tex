...
The noise must be additive!

The Wiener filter is a linear filter designed to minimize the MSE
between a filtered signal $\hat{\mathbf{x}}$ and true signal
$\mathbf{x}$, which can be corrupted by some linear transformation
(such as a blurring) and by noise. In the frequency domain this means
that
\begin{equation}
  \hat{\mathbf{X}} = \mathbf{H}\mathbf{X}+\mathbf{N},
  \label{eq:Weiner_degradation_model}
\end{equation}
where $\mathbf{H}$ if the DFT of the linear transformation, and
$\mathbf{N}$ if the DFT of the noise. Notice that we have supposed
that the noise is additive.

Because in denoising we suppose also the $\mathbf{H}=1$,
Eq.~\ref{eq:Weiner_degradation_model} becomes
\begin{equation}
  \hat{\mathbf{X}} = \mathbf{X}+\mathbf{N}.
  \label{eq:Weiner_noise_model}
\end{equation}

The transfer function of the Weiner filter is
\begin{equation}
  W = \frac{\text{PSD}(x)}{\text{PSD}(x)+\text{PSD}(n)}
\end{equation}
