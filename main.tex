...
\begin{abstract}
  We perform an objective comparison between some of the most used
  denoising algoritms in the field of Electron Microscopy. Two cases
  are considered: (1) when we have a denoised version of the signal to
  compare, and (2) when only a noisy version of the signal is known.
\end{abstract}
...
\section{Common sources of noise in microscopy}
...
As happen with quantization noise, in all capturing devices, the
thermal noise degrades the quality of the signals. Thermal noise is
generally modeled as additive zero-mean Gaussian noise
...
Speckle noise is generated in optical devices that use coherent light sources (lasers) \cite{kumar2021speckle}. Speckle noise is signal-dependent, so its variance changes with the intensity of the true image. It has been modeled as zero-mean multiplicative Gaussian noise \cite{} and as Rice noise \cite{}.
...
Multiplicative zero-mean Gaussian noise, modeled as
  ...
  Also known by photon noise \cite{zhou2020wirtinger}
Modeled as Poisson noise
...
Having two tensors $\mathbf{X}$ and $\mathbf{Y}$,
\begin{equation}
  \text{RMSE}(\mathbf{X},\mathbf{Y}) := \sqrt{\frac{1}{N}\sum_{i=1}^N(\mathbf{X}_i - \mathbf{Y}_i)^2},
\end{equation}
where $N$ is the number of samples in one of the tensors (that must
have the same number of samples). Notice that the RMSE expresses the
distortion in the same units as the samples.
...
We can approximate ${\mathbf X}$ by
