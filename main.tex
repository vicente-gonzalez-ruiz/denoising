...
are related to one another. The PPC is given by
\begin{equation}
...
\begin{figure}
  ...
  \caption{More SFRC curves of \emph{artificially-noisy} images. The
    parameters for SPRS are the same than the obtained the (true-)
...
removing noise, as it is our case, we will speak about Wiener
Denoising (WD).

To use WD, the next conditions must be satisfied:
\begin{enumerate}
...
Under these assumptions, WD minimizes the MSE between the output of
the filter $\tilde{\mathbf{x}}$ (the filtered signal) and the
(unknown) clean signal $\mathbf{x}$,
\begin{equation}
  \mathbb{E}[(\tilde{\mathbf{x}} - \mathbf{x})^2].
  \label{eq:WD_objective}
\end{equation}
Because $\mathbf{x}$ is seldom known, the WD uses the statistics of
$\hat{\mathbf{x}}$ (the noisy signal) to approximate to
Eq.~\ref{eq:WD_objective} (in other words, we can only estimate
$\tilde{\mathbf{x}}$). It can be demostrated that such estimation can
be found as
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:WD_frequency_response}
\end{equation}
is the frequency response of the Wiener filter (its transfer
function). In the signal domain, if the noise is ZAWG, the
Eq.~\ref{eq:WD_frequency_response} becomes
\begin{equation}
  \mathbf{h}_k = \frac{\text{Var}(\hat{\mathbf{X}}_k)}{\text{Var}(\hat{\mathbf{X}}_k) + N\sigma^2},
  \label{eq:WD_coeffs}
\end{equation}
where $\text{Var}(\hat{\mathbf{X}}_k)$ is the variance of the $k$-th
Fourier coefficient of the $\text{DFT}(\hat{\mathbf{x}})$, $N$ is the
length of the noisy signal, and $\sigma^2$ is an estimation of the
variance of the noise.

Notice that $\text{len}(\mathbf{h})=N$, and that to compute the
variances of the Fourier coefficients, we need at least two instances
of $\hat{\mathbf{x}}$. Neither such a large value of $N$ (filters that
are too large would not adapt to local features) nor only two
instances of the noisy signal (the variance values would have very
little statistical significance) are adequate in a real
experiment. For this reason, the signal is processed by windows. For
these reasons, the implementation offered in Scipy
(\href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.wiener.html}{\texttt{scipy.signal.wiener}})
requires two arguments:
\begin{enumerate}
\item A window size $w$, a parameter that should take into
  consideration the details of the signal.
\item A noise variance $\sigma$, which by default is
  $\sigma=\mathbb{E}[\text{Var}(\hat{\mathbf{x}}_{\{i\}})]$, where
  $\hat{\mathbf{x}}_{\{i\}}$ is the current window.
\end{enumerate}
...
The power spectrum shows how much energy a signal has at different
frequencies. Therefore, if $\mathbf{S}$ is the DFT of $\mathbf{s}$
(with $N$ samples),
\begin{equation}
  \frac{1}{N}|S|^2
\end{equation}
is the power spectrum of $\mathbf{s}$.

For real-valued digital signals, the spectrum is symmetric about
...
$\mathbf{x}$,
$\text{PSD}(\mathbf{x})$, is the DFT of its auto-correlation function
$r(\mathbf{x},\mathbf{x})$:
\begin{equation}
  \text{PSD}(\mathbf{x}) = \mathcal{F}(r(\mathbf{x},\mathbf{x})) = \sum_l r(\mathbf{x},\mathbf{x})_le^{-2\pi jfl},
\end{equation}
and, the auto-correlation function can be obtained as the inverse DFT
of the PSD (see the Wiener–Khinchin theorem, Appendix \ref{ape:WKT}):
\begin{equation}
...
theorem or the Wiener–Khinchin–Einstein theorem) states that the
...
other. Similarly, the auto-correlation is the inverse Fourier
...
In the context of discrete-time signals, the cross-correlation
measures the similarity between two signals $\mathbf{x}$ and
$\mathbf{y}$ as a function of the time lag $l\in\mathcal{N}$ of one
relative to the other. It is defined as
\begin{equation}
...
\label{ape:auto-correlation}
%{{{

The auto-correlation of a digital signal is another digital signal that
...
infinite length, the
autocorrelation signal at a lag $l$ is defined as:
\begin{equation}
...
Auto-correlation can be computed as the inverse Fourier transform of
...
\section{Variance}
The variance of a random variable $X$ is a measurement of its
dispersion. It is defined as the expected value of the squared
...
Let's say you have a (multivariate) dataset with $n$ observations and
$p$ variables, which can be represented as a matrix $X$ of size
$n\i¡times p$. The covariance matrix, often denoted by $\Sigma$, will
be a $p\times p$ matrix.

The diagonal elements of $\sigma$ is the
variance of the $i$-th variable:
\begin{equation}
  \text{Var}(X_i)=\mathbb{E}[(X_i-\mathbb{E}[X_i])^2].
\end{equation}

For the off-diagonal elements (he element at position $(i,j)$ (where $i\neq j$)
represents the covariance between the $i$-th and the $j$-th
variables:
\begin{equation}
  \text{Cocv}(X_i,X_j)=\mathbb{E}[(X_i-\mathbb{E}[X_i])(X_j-\mathbb{E}[X_j])]
\end{equation}
Covariance measures the degree to which two variables
change together.
