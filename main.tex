...
%{{{
  ...
  cannot be seen with the naked eye, and for this, the specimens must
  be irradiated. Unfortunately, the radiation also degrades the
  specimen. To minimize this degradation, the radiation doses are
  ...
  algoritms commonly used in microscopy of biological specimens.
...
The DFT of a digital signal $\mathbf{x}$ of lenght $N$ is defined as
\begin{equation}
  \mathbf{X}_f=\sum_n\mathbf{x}_ne^{-2\pi jfn},
\end{equation}
where $j=\sqrt{-1}$, and $f$ denotes frequency. The inverse DFT is
\begin{equation}
  \mathbf{x}_n=\frac{1}{N}\sum_f\mathbf{X}_fe^{2\pi jnf}.
\end{equation}
...
The Wiener-Khinching theorem (also known as the Wiener–Khintchine theorem
  or the Wiener–Khinchin–Einstein theorem) states that the cross-correlation of two
wide-sense stationary random processes is the inverse Fourier
transform of the product of the power spectral tensity of one process
and the conjugate of the power spectral density of the
...
Auto-correlation can be computed as the inverser Fourier transform of the power spectral density.
