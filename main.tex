...
\begin{enumerate}
\item The variance of the noise $\sigma$, which by default is
  $\sigma=\mathbb{E}[\mathbb{V}(\hat{\mathbf{x}}_{\{i\}})]$, where
  $\hat{\mathbf{x}}_{\{i\}}$ is the block used in the denoising
  process.
\item A window size $w$ which controls the block size in which the
  energy of the noise is estimated.
\end{enumerate}

Another alternative to find the transfer function of the Wiener filter
is to use the SFC \cite{verbeke2024self}. In this case,
...
auto-correlation function $r(\mathbf{x},\mathbf{x})$:
\begin{equation}
