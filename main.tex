...
\usepackage{amsfonts}
\usepackage{amssymb}
%\usepackage[draft]{graphicx}\graphicspath{{figs}{notebooks}{.}}
...
differences between two images $\mathbf{X}$ and $\mathbf{Y}$. The
metric returns a real number between $[-1, 1]$, $-1$ representing the
perfect dis-similarity case, $0$ no similarity, and $1$ perfect
similarity. The SSIM index is determined (splitting the input images
into $M$ non-overlapping blocks) with
\begin{equation}
...
\end{equation}
where $\mathbf{X}$, $\hat{\mathbf{Y}}$ are the original and denoised
images, respectively, $\overline{\mathbf{x}}_j$,
...
deviations of the blocks of $\mathbf{X}_j$ and $\hat{\mathbf{Y}}_j$,
...
$||f_i(\mathbf{X}) - f_i(\mathbf{Y})||_2$ measures the distance
between the features at that layer.
...
\end{equation}
Notice that the L2 norm and Mean Squared Error (MSE) are closely
related concepts, because
\begin{equation}
  ||\mathbf{Y} - \mathbf{X}||_2^2 = J\cdot\text{MSE}(\mathbf{Y} - \mathbf{X}).
\end{equation}
...
Characterization of noise (in terms of power, statistical model, and
spectral profile, etc.) can provide valuable insights for determining
the optimal operational parameters of denoising algorithms. However,
this approach typically necessitates the availability of at least two
independent noisy realizations of the underlying clean signal. This
requirement often presents a significant challenge, particularly in
contexts where samples are susceptible to degradation from microscope
radiation, thereby limiting the feasibility of acquiring multiple such
instances. Notice that, by definition, the noise is uncorrelated
between instances.

One way to simulate having two or more noisy instances of the same
clean signal is to distribute the pixels/voxels across two or more
images/volumes (see Appendices \ref{ape:EOS}, \ref{ape:CBS},
\ref{ape:ICBS}, \ref{ape:SCBS}, and \ref{ape:SPRS}). Unfortunately,
the splitting reduces the spatial resolution at which the signal/noise
parameters can be successfully estudied.
...
\begin{figure}
  ...
  \resizebox{1.0\textwidth}{!}{
    ...
    \begin{tabular}{cc}
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Confocal_FISH_clean.ipynb}{\includegraphics{Confocal_FISH_clean.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/TwoPhoton_MICE_clean.ipynb}{\includegraphics{TwoPhoton_MICE_clean.pdf}} \\
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Confocal_FISH_clean_SFRC.ipynb}{\includegraphics{Confocal_FISH_clean_SFRC.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/TwoPhoton_MICE_clean_SFRC.ipynb}{\includegraphics{TwoPhoton_MICE_clean_SFRC.pdf}}
    \end{tabular}
  }
  \caption{SFRC (Self Fourier Ring Correlation) of two clean images
    ...
    FSC(SPRS($\mathbf{X}$), SPRS($\mathbf{X}$))), and SPRS2 (computing
    the FSC($\mathbf{X}$, SPRS($\mathbf{X}$))). $f_s$ represents the
    sampling frequency.\label{fig:SFC_vs_splitting_clean}}
\end{figure}
...
\begin{figure}
  ...
  \resizebox{1.0\textwidth}{!}{
    ...
    \setlength{\tabcolsep}{0ex}      % Adjust column spacing in the table    
    \begin{tabular}{cc}
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Confocal_FISH_noisy.ipynb}{\includegraphics{Confocal_FISH_noisy.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/TwoPhoton_MICE_noisy.ipynb}{\includegraphics{TwoPhoton_MICE_noisy.pdf}} \\
      \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/Confocal_FISH_noisy_SFRC.ipynb}{\includegraphics{Confocal_FISH_noisy_SFRC.pdf}} & \href{https://nbviewer.org/github/vicente-gonzalez-ruiz/denoising/blob/main/notebooks/TwoPhoton_MICE_noisy_SFRC.ipynb}{\includegraphics{TwoPhoton_MICE_noisy_SFRC.pdf}}
    \end{tabular}
  }
  \caption{SFRC (Self Fourier Ring Correlation) of two
    ...
    (FSC($\hat{\mathbf{X}}^{(1)}$, $\hat{\mathbf{X}}^{(2)}$)). $f_s$
    represents the sampling
    frequency.\label{fig:SFC_vs_splitting_noisy}}
\end{figure}
...
\begin{figure}
  ...
  \caption{SFRC (Self Fourier Ring Correlation) of two MPG
    ...
    $\hat{\mathbf{X}}^{(2)}$)). $f_s$ represents the sampling
    frequency.\label{fig:SFC_vs_splitting_artificial}}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:search_eta}
\end{equation}
where $\beta$ is the attenuation used in the stop-band (see
...
\begin{equation}
  \tau^*_{\text{SFC}} = \frac{0.141}{2\eta^*},
  \label{eq:tau_VS_eta_empirical_SFC}
\end{equation}
(and not exactly the fitting expression obtanied in the
...
information. Notice that, to determine $\eta^*$ we have supposed that
when the correlation is smaller than $1/\sqrt{2}$ (see
Appendix~\ref{ape:tau_VS_eta}), %$0.5$,
the SNR is not high enough to keep visible the corresponding
frequencies.
...
\cite{farneback2003two}, but among all of then $w$ (the size of the 2D
window used by the OF estimator) has the highest impact on the
performance of the filtering process: higher $w$ values increase the
smootness of the OF fields and this tends to increase blurring, but
also, at the same time, to decrease the noise. Therefore, it is
expected that there is an optimal value for $w$ depending on the
amount of noise and the content of the volume.
...
two effects (obviously, if $\hat{\tau}$ remains constant):
\begin{enumerate}
...
\begin{figure}
  ...
  \caption{Impact of $k$ in the frequency response of a digital
    Gaussian kernel, when $k$ and $\hat{\tau}$ are related according
...
\begin{align}
  ...
  \mathbf{X}^{(\text{OO})} & = (\mathbf{X}_{\{e\},:})_{:,\{o\}}, \text{(pixels with odd row and odd column indices)}
\end{align}
where
\begin{align}
  \{o\} = \{i\mid M(i)=1\}, \\
...
\bibliography{signal_processing,microscopy,denoising,motion_estimation,image_compression}
