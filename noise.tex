...
Unfortunately, in general we don't know either $\mathbf{s}$ or
$\mathbf{n}$. However, we can suppose that (see
Appendix~\ref{sec:power_signal}):
\begin{equation}
  P(\mathbf{s}) = \mathbb{E}(\hat{\mathbf{s}}^2) \approx \mathbb{E}(\hat{\mathbf{s}})^2,
\end{equation}
and that (see
  Appendix~\ref{sec:power_noise}):
\begin{equation}
  P(\mathbf{n}) = \mathbb{V}(\hat{\mathbf{s}}).
\end{equation}

Therefore, Eq.~\ref{eq:SNR} becomes
\begin{equation}
  \text{SNR} \approx \frac{\mathbb{E}(\hat{\mathbf{s}})^2}{\mathbb{V}(\hat{\mathbf{s}})}.
  \label{eq:SNR-distribution}
\end{equation}
...
\subsection{\gls{SNR} of the model}
Using Eq.~\ref{eq:SNR_distribution}, and Eq.~\ref{eq:E_constant}, %that
%\begin{equation}
%  \mathbb{E}(\mathbf{s})=\mathbb{E}(\hat{\mathbf{s}}) = \mathbf{s},
%\end{equation}
\begin{equation}
...
\section{Relation between the power of a clean signal and a noisy version of it}
\label{sec:power_signal}
If $\mathbb{E}(\mathbf{n})=0$, the power of the original signal is the
same than the mean of the square of the noisy signal:
...
\begin{alignat*}{2}
  P(\mathbf{s}) & = \mathbb{E}(\mathbf{s}^2) \\
                ...
                & = \mathbb{E}(\hat{\mathbf{s}}^2).
\end{alignat*}

\section{Relation between the power of the noise and the noisy signal}
