...
If we are interested in analyze the noise, we can suppose that
$\mathbb{V}(\mathbf{s})=0$ (the signal is constant) and that
$\mathbb{E}(\mathbf{n}))=0$ (the mean of the noise is zero). Under
these assumptions, we get that
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:E_Poisson}
\end{equation}
Therefore, as it happens with the \gls{ZAWG} model, $\hat{\mathbf{s}}$
is an unbiased estimator of $\mathbf{s}$ (the expected value for each
noisy $i$-th pixel is the clean $i$-th pixel).
...
$\gamma$. Therefore, considering the Eq.~\ref{eq:PN}, we must reduce
...
\gls{MPG} noise \cite{meiniel2018denoising}. The genration of \gls{MPG} noise can be modeled as
as
\begin{equation}
