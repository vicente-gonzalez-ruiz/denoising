...
\item The noise samples follow a Gaussian (normal) distribution,
  centered at zero:
  \begin{equation}
    {\mathbf n}\sim{\mathcal N}(\mu=0,\sigma^2),
  \end{equation}
  and exhibit a standard deviation of $\sigma$. 
\end{enumerate}

Notice that in this Gaussian noise model,
\begin{equation}
  \mathbb{E}(\hat{\mathbf{s}}) = \mathbb{E}(\mathbf{s}),
  \label{eq:E_ZAWG}
\end{equation}
and
\begin{equation}
...
the clean $i$-th pixel. In other words, $\hat{\mathbf{s}}$ is an
unbiased estimator or $\mathbf{s}$.
...
\end{equation}
where $\alpha\in[0,~1]$ controls the ratio of both types of noise
...
our experiments, $\alpha=0.5$. Notice that, as reported in
\cite{foi2008practical}, this ratio should be described in the
datasheet of the imaging hardware. Otherwise, estimated from the noisy
images.
