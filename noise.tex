...
noise (Poissonian and Gaussian). Therefore, for $\alpha > 0$,\footnote{If $\sigma=0$ and
...
our experiments, $\alpha=0.5$. As reported in \cite{foi2008practical},
this ratio should be representative of typical imaging sensors.

It should be obvious to see that in the noise model described in
Eq.~\ref{eq:MPG_noise_model} we have that
\begin{equation}
  \mathbb{E}(\hat{\mathbf{s}}) = \mathbf{s}/\gamma,
  \label{eq:E_MPG_0}
\end{equation}
and that
\begin{equation}
  \mathbb{V}(\hat{\mathbf{s}}) = \mathbf{s}/\gamma + \sigma.
  \label{eq:V_MPG_0}
\end{equation}
In particular, for $\hat{\mathbf{s}}=\mathbf{0}$ (a constant signal
with zeros), Equations \ref{eq:E_MPG_0} and Eq.~\ref{eq:V_MPG_0}
become
\begin{equation}
  \mathbb{E}(\mathbf{0}) = 0
\end{equation}
and
\begin{equation}
  \mathbb{V}(\mathbf{0}) = \sigma,
\end{equation}
and for $\hat{\mathbf{s}}=\mathbf{1}$ (a constant signal with ones)
\begin{equation}
  \mathbb{E}(\mathbf{1}) = 1/\gamma
\end{equation}
and
\begin{equation}
  \mathbb{V}(\mathbf{1}) = 1/\gamma + \sigma,
\end{equation}
