...
If $\mathbf{s}$ is the clean signal and $\hat{\mathbf{s}}$ is its
noisy version, the \gls{SNR} is defined as the ratio between the power
(see Eq.~\ref{eq:power_discrete_signal}) and
Eq.~\ref{eq:power_as_expectation}) of the clean signal and the power
of the noise
\begin{equation}
  \text{SNR} = \frac{P(\mathbf{s})}{P(\mathbf{n})},
\end{equation}
where
\begin{equation}
...
Considering now Eq.~\ref{eq:power_mean_variance}, we come to
\begin{equation}
  \text{SNR} = \frac{\mathbb{V}(\mathbf{s}) + \mathbb{E}(\mathbf{s})^2}{\mathbb{V}(\mathbf{n}) + \mathbb{E}(\mathbf{n})^2}.
\end{equation}

Without loss of generality, we can suppose that
$\mathbb{V}(\mathbf{s})=0$ (the signal is constant) and that
$\mathbb{E}(\mathbf{n}))=0$ (the mean of the noise is zero). Under these assumptions, we get that
\begin{equation}
  \text{SNR} = \frac{\mathbb{E}(\mathbf{s})^2}{\mathbb{V}(\mathbf{n})} = \frac{\mathbb{E}(\mathbf{s})}{\sqrt{\mathbb{V}(\mathbf{n})}}.
  \label{eq:SNR_distribution}
\end{equation}
...
\gls{ZAWG} noise is a type of random noise
characterized by:
\begin{enumerate}

\item The noise is added to the signal:
  \begin{equation}
...
\item The nose is independent of the signal:
  \begin{equation}
...
\item The noise samples $\mathbf{n}_i$ follow a Gaussian (normal)
  distribution, centered at zero ($\mathbb{E}(\mathbf{n}_i) = 0$), and
  exhibit a standard deviation of $\sigma$:
  \begin{equation}
    {\mathbf n}_i\sim{\mathcal N}(\mu=0,\sigma^2).
  \end{equation}

\item The noise is uncorrelated:
  \begin{equation}
    \mathbb{E}(\mathbf{n}_i\mathbf{n}_j) = 0 \quad \text{for} \quad i\neq j.
  \end{equation}

\item The noise has a constant \gls{PSD} over all frequencies (in
  other words, the spectrum of \gls{ZAWG} noise is flat).
...
In this noise model, considering that $\mathbf{s}$ is a constant (the
clean image is fixed for each adquisition of the image $\mathbf{s}$),
\begin{equation}
  \mathbb{E}(\hat{\mathbf{s}}) = \mathbb{E}(\mathbf{s} + \mathbf{n}) = \mathbb{E}(\mathbf{s}) + \mathbb{E}(\mathbf{n}) = \mathbf{s} + \mathbf{0} = \mathbf{s}.
  \label{eq:E_ZAWG}
\end{equation}
This basically implies that if we take an infinite number of noisy
instances of the same biological specimen, and these instances are
affected by ZAWG noise, then the expected value for each noisy $i$-th
pixel is the clean $i$-th pixel. In other words, $\hat{\mathbf{s}}$ is
an unbiased estimator or $\mathbf{s}$.

\subsection{Variance}
In the case of the variance, we have that
\begin{equation}
...
dependency betweeen both variances. This is 
...
\subsection{\gls{SNR}}
Now, using Eq.~\ref{eq:SNR_distribution}, we come to
\begin{equation}
  \text{SNR}_{\mathcal{N}(\mu,\sigma^2)} = \frac{\mu}{\sigma}.
\end{equation}
...
(e.g., photons and electrons). It is particularly relevant in
low-radiation imaging, and quantum physics.

Signal-dependent Poisson noise can be modeled as
\begin{equation}
  \hat{\mathbf{s}} = \frac{\mathbf{n}}{\gamma},~\mathbf{n}\sim\mathcal{P}(\lambda=\gamma\mathbf{s}),
  \label{eq:PN}
\end{equation}
where $\gamma$ controls the variance (the higher $\gamma$,
...
$\hat{\mathbf s}$ will have a higher mean and variance,
and therefore a higher noise power \cite{meiniel2018denoising}. This
also implies that the standard deviation of the noise depends on the
expectation of the noisy signal, which depends on the expectation of
the clean signal \cite{foi2008practical}.
...
Notice that this expectation is a vector (with the same shape that
$\mathbf{s}$). Equations \ref{eq:EPNi} and \ref{eq:E_Poisson} tell us
that, for example, if we take an infinite number of noisy instances of
the same biological specimen, and these instances are affected by
Poisson noise, then the expected value for each noisy $i$-th pixel is
the clean $i$-th pixel. As happens with the ZAWG noisy model,
$\hat{\mathbf{s}}$ is an unbiased estimator or $\mathbf{s}$.

In the case of the variance,
\begin{equation}
  \mathbb{V}(\hat{\mathbf{s}}) = \mathbb{V}\left(\frac{\mathbf{n}}{\gamma}\right) = \frac{1}{\gamma^2}\mathbb{V}(\mathbf{n}) = \frac{1}{\gamma^2}\gamma\mathbf{s} = \mathbf{s}/\gamma,
  \label{eq:V_Poisson}
\end{equation}
which indicates that the variance of the noisy signal depends linearly
on the expectation of the clean signal
\cite{foi2008practical}. Obviously, this is a signal-dependent noisy
model.
...
Using Eq.~\ref{eq:SNR_distribution},
\begin{equation}
  \text{SNR}_{\mathcal{P}(\gamma\mathbf{s})} = \frac{\mathbf{s}}{\sqrt{\mathbf{s}/\gamma}} = \sqrt{\gamma\mathbf{s}}.
  \label{eq:SNR_Poisson}
\end{equation}
Notice that the SNR grows with $\mathbf{s}$ and with
$\gamma$. Therefore, using the Eq.~\ref{eq:PN}, we must reduce
$\gamma$ to increase the noise.
...
\gls{MPG} noise \cite{meiniel2018denoising}, generated as
\begin{equation}
...
(Poissonian and Gaussian). Therefore, for $\alpha > 0$,\footnote{If
  $\sigma=0$ and $\alpha=0$, both types of noise vanish and
  $\hat{\mathbf{s}}=\mathbf{s}$.} the noise is signal-dependent and
it's spectrum resembles the spectrum of $\mathbf{x}$ (by default, in
our experiments, $\alpha=0.5$), i.e. both types of noise are present
in the same proportion. Notice that, as reported in
\cite{foi2008practical}, this ratio should be described in the
datasheet of the imaging hardware, or if this information is not
known, estimated from the noisy images.
...
\section{Self Fourier Correlation (SFC)}
%{{{
