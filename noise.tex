...
version of the clean signal is available (neither
the clean signal nor the noise are known).
...
\begin{equation}
  \text{SNR} = \frac{P(\mathbf{s})}{P(\mathbf{n})} = \frac{\mathbb{E}(\mathbf{s}^2)}{\mathbb{E}(\mathbf{n}^2)} = \frac{\mathbb{E}(\mathbf{s})^2+\mathbb{V}(\mathbf{s})}{\mathbb{E}(\mathbf{n})^2+\mathbb{V}(\mathbf{n})},
  \label{eq:SNR}
\end{equation}
...
\begin{equation}
  \mathbb{V}(\mathbf{s}) = 0,
\end{equation}
because we are interested in measuring the power of the clean signal,
and considering only its mean can be a good approximation to this, and
that
\begin{equation}
  \mathbb{E}(\mathbf{n}) = 0,
\end{equation}
because this the most common in real imaging systems, Eq.~\ref{eq:SNR}
becomes
\begin{equation}
  \text{SNR} \approx \frac{\mathbb{E}(\mathbf{s})^2}{\mathbb{V}(\mathbf{n})}.
\end{equation}

Unfortunately, in general we don't know either $\mathbf{s}$ or
$\mathbf{n}$. However, using that
\begin{equation}
  \mathbb{E}(\mathbf{s}^2) = \mathbb{E}(\hat{\mathbf{s}}^2),
\end{equation}
(see Appendix~\ref{sec:power_signal},
 that 
\begin{equation}
  \mathbb{E}(\mathbf{s}^2)^2 = \mathbb{E}(\hat{\mathbf{s}}^2)^2,
\end{equation}
(see Appendix~\ref{sec:power_noise}) and that
\begin{equation}
  \mathbb{V}(\mathbf{n}) = \mathbb{V}(\hat{\mathbf{s}}-\mathbf{s}) = \mathbb{V}(\hat{\mathbf{s}}) + \mathbb{V}(\mathbf{s}) = \mathbb{V}(\hat{\mathbf{s}}),
\end{equation}
because $\mathbb{V}(\mathbf{s})=0$, we finally get that
\begin{equation}
  \text{SNR} \approx \frac{\mathbb{E}(\hat{\mathbf{s}})^2}{\mathbb{V}(\hat{\mathbf{s}})}.
  \label{eq:SNR-distribution}
\end{equation}
(again, we are interested in the power of the signal, not in its
\end{comment}
...
\item \textbf{White spectrum}: $\mathbf{n}$ has a constant \gls{PSD} over all frequencies (in
  other words, the spectrum of $\mathbf{n}$ is flat).
...
Considering Eq.~\ref{eq:SNR}: %and Eq.~\ref{eq:E_ZAWG}, we get that
\begin{equation}
...
data represent counts of discrete events (e.g., photons, electrons,
etc.). Poisson noise becomes particularly relevant in low-radiation
imaging.
...
the instances of a single clean\footnote{Generated by
