...
with zeros), Equations \ref{eq:E_ZAWG} and Eq.~\ref{eq:V_ZAWG}
reduce to
\begin{equation}
...
and therefore a higher noise level \cite{meiniel2018denoising}. This
...
and
\begin{equation}
  \mathbb{V}(\hat{\mathbf{s}}) = \mathbb{V}(\mathbf{s}) = \mathbb{E}(\mathbf{s}).
\end{equation}
...
It is easy to see that in the noise model described in
Eq.~\ref{eq:MPG_noise_model} we have that
\begin{equation}
  \mathbb{E}(\hat{\mathbf{s}}) = (1-\alpha)\mathbb{E}(\mathbf{s}) + \alpha\mathbb{E}(\gamma\mathbf{s})/\gamma = (1-\alpha)\mathbb{E}(\mathbf{s}) + \alpha\mathbb{E}(\mathbf{s}) = \mathbb{E}(\mathbf{s}),
  \label{eq:E_MPG}
\end{equation}
and that
\begin{equation}
  \mathbb{V}(\hat{\mathbf{s}}) = (1-\alpha)\mathbb{V}(\mathbf{s}) + \alpha\mathbb{V}(\gamma\mathbf{s})/\gamma = (1-\alpha)\sigma^2 + \alpha\gamma\mathbf{s}/\gamma = (1-\alpha)\sigma^2 + \alpha\mathbf{s}.
  \label{eq:V_MPG}
\end{equation}
In particular, for $\mathbf{s}=\mathbf{0}$ (a constant signal
with zeros), Equations \ref{eq:E_MPG} and Eq.~\ref{eq:V_MPG}
become
\begin{equation}
  \mathbb{E}(\mathbf{\hat{\mathbf{s}}}) = 0,
\end{equation}
...
and
\begin{equation}
  \mathbb{V}(\hat{\mathbf{s}}) = (1-\alpha)\sigma^2 + \alpha.
\end{equation}
